...
# --- [1. 세션 상태 및 설정 통합 관리] ---
# 메뉴 이름과 안내 문구를 요청하신 최종 버전으로 고정합니다.
...
# [수정] 상단 부제목 출력 (중복 방지를 위해 sub_menu2를 여기서만 출력)
...
if selected_menu == "🏠 홈 (대시보드)":
    ...
    edited_df = st.data_editor(df_acc, num_rows="dynamic", use_container_width=True, key="acc_editor_final_v6")
    ...
    # [중복 제거] 파란 박스(st.info)를 삭제하고 상단 부제목만 유지함
