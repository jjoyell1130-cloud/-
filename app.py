...
class SimplePDF(FPDF):
    def __init__(self, title, biz):
        ...
        if os.path.exists(font_path):
            try:
                ...
                self.font_name = 'helvetica' # 대체 폰트
        else:
    ...
    def header(self):
        ...
        # 최신 fpdf2 문법: ln=True 대신 new_x, new_y 사용
...
if curr == M2:
    uploaded_file = st.file_uploader("📊 엑셀 파일 업로드", type=['xlsx'], key="pdf_uploader")
    if uploaded_file:
            ...
            for i, d_type in enumerate(['매출', '매입']):
                with cols[i]:
                    ...
                    if not sub_df.empty:
                        ...
                        # [핵심 수정] pdf.output()을 bytes로 변환하여 전달
