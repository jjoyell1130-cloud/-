...
def extract_data_from_pdf(files):
    data = {"매출액": "0", "매입액": "0", "세액": "0", "결과": "납부"} [cite: 3]
    ...
    for file in files:
        with pdfplumber.open(file) as pdf:
            ...
            if any(k in file.name for k in ["신고서", "접수증"]):
                tax_match = re.search(r"(납부할세액|차가감세액|합계세액|세액합계)[:]*([-]*[\d,]+)", full_text_clean) [cite: 4]
                if tax_match:
                    raw_amt = tax_match.group(2).replace(",", "")
                    amt = int(raw_amt)
                    data["결과"] = "환급" if "환급" in full_text_clean or amt < 0 else "납부"
                    data["세액"] = f"{abs(amt):,}" [cite: 5]
            ...
            if (is_sales or is_purchase) and pages:
                ...
                for line in reversed(last_page_lines):
                    if any(k in line for k in ["합계", "총계", "누계"]): [cite: 6]
                        amts = re.findall(amt_pattern, line)
                        if amts:
                            if is_sales: data["매출액"] = amts[0] [cite: 7]
...
def make_pdf_stream(data, title, biz_name, date_range):
    ...
    for i in range(len(data)):
        if i % rows_per_page == 0:
            ...
            c.setFont(FONT_NAME, 18); c.drawCentredString(width/2, height - 60, title) [cite: 9]
            c.setFont(FONT_NAME, 10); c.drawString(50, height - 90, f"회사명 : {biz_name}") [cite: 10]
            c.drawString(50, height - 105, f"기  간 : {date_range}") 
            c.drawRightString(width - 50, height - 90, f"페이지 : {p_num}")
            yh = 680 
            c.setLineWidth(1.2); c.line(40, yh + 15, 555, yh + 15) [cite: 11]
            c.setFont(FONT_NAME, 9); c.drawString(45, yh, "번호"); c.drawString(90, yh, "일자") [cite: 12]
            c.drawString(180, yh, "거래처(적요)"); c.drawRightString(420, yh, "공급가액") [cite: 13]
            c.drawRightString(485, yh, "부가가치세"); c.drawRightString(550, yh, "합계") [cite: 14]
            c.line(40, yh - 8, 555, yh - 8); y_start = yh - 28 [cite: 15]
        row = data.iloc[i]
        ...
        if is_summary:
            c.setFont(FONT_NAME, 9); c.drawString(90, cur_y, str(row.get('거래처', row.get('번호', '')))) [cite: 16]
            c.line(40, cur_y + 16, 555, cur_y + 16); c.line(40, cur_y - 7, 555, cur_y - 7) [cite: 17]
        else:
            actual_item_count += 1
            ...
            c.setStrokeColor(colors.lightgrey); c.line(40, cur_y - 7, 555, cur_y - 7); c.setStrokeColor(colors.black) [cite: 18, 19]
...
with st.sidebar:
    st.markdown("### 📁 Menu")
    for k in ["menu_0", "menu_1", "menu_2", "menu_3"]: [cite: 21]
...
if curr == st.session_state.config["menu_0"]:
    ...
    # 전표 입력 가이드 (차변/대변 요약 및 순번 삭제, 계정/코드 분리)
...
# Menu 1: 마감작업
    ...
    if all_up:
        ...
        biz = all_up[0].name.split("_")[0] if "_" in all_up[0].name else all_up[0].name.split(" ")[0] [cite: 23]
...
# Menu 2: PDF 변환
    ...
    if f_pdf:
        df_all = pd.read_excel(f_pdf); biz_name = f_pdf.name.split(" ")[0] [cite: 24]
        try:
...
# Menu 3: 카드 분리
    ...
    if card_up:
        ...
        clean_name = re.sub(r'\(.*?\)', '', raw_fn.replace("위하고_수기입력_", "")).strip() [cite: 27]
        ...
        dt_col = next((c for c in df.columns if '이용일' in str(c)), None) [cite: 28]
        if dt_col:
        ...
        amt_col = next((c for c in df.columns if any(k in str(c) for k in ['매출금액', '금액', '합계'])), None) [cite: 29]
        if num_col and amt_col:
            z_buf = io.BytesIO()
            with zipfile.ZipFile(z_buf, "a", zipfile.ZIP_DEFLATED) as zf:
                for c_num, group in df.groupby(num_col):
                    if pd.isna(c_num): continue [cite: 30]
                    up_df = group.copy()
                    total_amt = pd.to_numeric(up_df[amt_col].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
                    up_df['공급가액'] = (total_amt / 1.1).round(0).astype(int) [cite: 31]
                    up_df['부가세'] = total_amt.astype(int) - up_df['공급가액']
                    excel_buf = io.BytesIO()
                    with pd.ExcelWriter(excel_buf, engine='xlsxwriter') as writer: [cite: 32]
