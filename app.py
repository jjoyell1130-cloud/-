...
# [업데이트] 요청하신 상세 차변 계정 리스트 (25개 항목)
if 'account_data' not in st.session_state:
    st.session_state.account_data = [
        {"구분": "차량", "주요 거래처": "유류대", "분류": "공제유무확인후 분류", "계정명": "차량유지비", "코드": "822"},
        {"구분": "여비", "주요 거래처": "편의점", "분류": "공제유무확인후 분류", "계정명": "여비교통비", "코드": "812"},
        {"구분": "매입", "주요 거래처": "다이소", "분류": "매입", "계정명": "소모품비", "코드": "830"},
        {"구분": "식대", "주요 거래처": "식당", "분류": "공제유무확인후 분류", "계정명": "복리후생비", "코드": "811"},
        {"구분": "매입", "주요 거래처": "거래처", "분류": "매입", "계정명": "상품", "코드": "146"},
        {"구분": "매입", "주요 거래처": "홈쇼핑, 인터넷구매", "분류": "매입", "계정명": "소모품비", "코드": "830"},
        {"구분": "차량", "주요 거래처": "주차장, 적은금액세금", "분류": "일반", "계정명": "차량유지비", "코드": "822"},
        {"구분": "여비", "주요 거래처": "휴게소", "분류": "공제유무확인후 분류", "계정명": "차량/여비교통비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "전기요금", "분류": "매입", "계정명": "전력비", "코드": "-"},
        {"구분": "유틸", "주요 거래처": "수도요금", "분류": "일반", "계정명": "수도광열비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "통신비", "분류": "매입", "계정명": "통신비", "코드": "814"},
        {"구분": "세금", "주요 거래처": "금융결제원", "분류": "일반", "계정명": "세금과공과", "코드": "-"},
        {"구분": "일반", "주요 거래처": "약국", "분류": "일반", "계정명": "소모품비", "코드": "830"},
        {"구분": "여비", "주요 거래처": "모텔", "분류": "일반", "계정명": "출장비/여비교통비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "캡스, 보안, 홈페이지", "분류": "매입", "계정명": "지급수수료", "코드": "831"},
        {"구분": "매입", "주요 거래처": "아울렛(작업복)", "분류": "매입", "계정명": "소모품비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "컴퓨터 AS", "분류": "매입", "계정명": "수선비", "코드": "820"},
        {"구분": "일반", "주요 거래처": "결제대행업체", "분류": "일반", "계정명": "소모품비", "코드": "830"},
        {"구분": "일반", "주요 거래처": "신용카드 알림", "분류": "일반", "계정명": "지급수수료", "코드": "-"},
        {"구분": "일반", "주요 거래처": "휴대폰 소액결제", "분류": "일반", "계정명": "소모품비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "매입 항목", "분류": "매입", "계정명": "상품", "코드": "146"},
        {"구분": "일반", "주요 거래처": "병원", "분류": "일반", "계정명": "복리후생비", "코드": "-"},
        {"구분": "일반", "주요 거래처": "금융결제원", "분류": "일반", "계정명": "소모품비", "코드": "-"},
        {"구분": "일반", "주요 거래처": "로카모빌리티", "분류": "일반", "계정명": "소모품비", "코드": "-"},
        {"구분": "매입", "주요 거래처": "소프트웨어 개발/공급", "분류": "지급수수료", "계정명": "지급수수료", "코드": "831"}
    ]
...
if selected_menu == "🏠 홈 (대시보드)":
    ...
    # [업데이트 반영]
    edited_df = st.data_editor(pd.DataFrame(st.session_state.account_data), num_rows="dynamic", use_container_width=True, key="acc_editor_final")
    if st.button("💾 리스트 저장"):
        st.session_state.account_data = edited_df.to_dict('records')
        st.success("25개의 계정 정보가 성공적으로 저장되었습니다.")
